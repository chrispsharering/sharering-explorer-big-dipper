...
export const TransactionRow = (props) => {
    ...
    return (props.isHomePage?
        <Row className="block-info">
            <Col xs={4} className="text-truncate"><Link to={"/transactions/"+tx.txhash}>{tx.txhash}</Link></Col>
            <Col>${numbro(tx.feeUsd).format({ mantissa: 2 })}</Col>
            <Col>{tx.block() ? moment(tx.block().time).fromNow() : ''}</Col>
        </Row>
    :
        <SentryBoundary><Row className={(tx.code)?"tx-info invalid":"tx-info"}>
            ...
            <Col xs={(!props.blockList)?6:8} md={(!props.blockList)?9:4} lg={2} className="fee"><i className="material-icons d-lg-none">monetization_on</i> {(tx.tx.value.fee.amount.length > 0)?tx.tx.value.fee.amount.map((fee,i) => {
                return <span className="text-nowrap" key={i}>{fee.amount} SHR</span>
